...
export async function GET(req: NextRequest) {
  ...
  // Full leaderboard
  if (!supabaseAdmin) return NextResponse.json({ rankings: [] });

  const { data, error } = await supabaseAdmin
    .from('rankings')
    .select('id, name, avatar, score, total_questions, percentage, streak, duration, created_at, device_id')
  ...
  const rankings = (data ?? []).map((r) => ({
    ...
    duration:       r.duration,
    date:           r.created_at,
    playerId:       r.device_id ?? undefined,
  }));
